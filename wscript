...
def set_options(opt):
    opt.tool_options('build', tooldir=join(opt.curdir, 'modules/build'))
...
def configure(conf):
    ...
    conf.env.append_value('CPPPATH_SIX_SAMPLES', os.path.join(conf.curdir, 'sio.lite', 'include'))
    conf.env.append_value('LIBPATH_SIX_SAMPLES', os.path.join(conf.blddir, conf.env.variant(), 'sio.lite'))
