...
{
    nrt_ListNode *node =
        (nrt_ListNode *) NRT_MALLOC(sizeof(nrt_ListNode));
    if (node == NULL)
    {
...
NRTAPI(NRT_BOOL) nrt_List_pushFront(nrt_List * this_list,
                                       NRT_DATA * data,
                                       nrt_Error * error)
{
...
NRTAPI(NRT_BOOL) nrt_List_pushBack(nrt_List * this_list,
    ...
    if (source)
    {
        ...
        while (nrt_ListIterator_notEqualTo(&iter, &end))
        {
            ...
            if (!nrt_List_pushBack(l, newData, error))
            {
                 ...
                 * NOTE - there is no way for us to destroy the NRT_DATA* chunks
...
NRTAPI(NRT_DATA *) nrt_List_remove(nrt_List * list,
                                  ...
                                  nrt_Error * error)
{
    ...
    {
        nrt_Error_init(error, "Invalid list index specified",
                            NRT_CTXT, NRT_ERR_INVALID_PARAMETER);
        return NRT_FAILURE;
    }
...
NRTAPI(NRT_BOOL) nrt_List_insert(nrt_List * list,
                                    nrt_ListIterator iter,
                                    NRT_DATA * data, nrt_Error * error)
{
    ...
    else
    {
        ...
        nrt_ListNode *new_node =
            nrt_ListNode_construct(iter.current->prev,
                                    iter.current,
                                    data,
                                    error);
                                   ...
                                   int index, nrt_Error * error)
{
    ...
    if (list)
    {
        nrt_ListIterator iter = nrt_List_begin(list);
        nrt_ListIterator end = nrt_List_end(list);
        for (i = 0; i < index && nrt_ListIterator_notEqualTo(&iter, &end);
                ++i)
            nrt_ListIterator_increment(&iter);
        ...
        {
            nrt_Error_init(error, "Object not found at index",
    ...
    {
        nrt_Error_init(error, "Invalid list -> NULL",
                        NRT_CTXT, NRT_ERR_INVALID_OBJECT);
        return NULL;
    }
}
