                           ...
                           ptrdiff_t offset)
{
    ...
    // Add to ordered map
    mOffsetMap.insert(std::pair<ptrdiff_t,size_t>(offset,numBytes));

    // Validate offset and size
    if (mOffsetMap.find(offset) != mOffsetMap.begin())
    {
        if (offset < (ptrdiff_t)((--mOffsetMap.find(offset))->first + (--mOffsetMap.find(offset))->second))
        {
            std::ostringstream ostr;
    ...
    {
        if ((ptrdiff_t)(offset + (numRows * numCols * numBytes)) > mOffsetMap.upper_bound(offset)->first)
        {
            std::ostringstream ostr;
            ostr << "Invalid size or offset of support array given for id: " << id;
            throw except::Exception(ostr.str());
