...
namespace cphd
{
const char CPHDXMLControl::CPHD_URI[] = "urn:CPHD:0.3";

CPHDXMLControl::CPHDXMLControl() :
    six::XMLParser(CPHD_URI, false, new logging::NullLogger(), true),
    mCommon(CPHD_URI, false, CPHD_URI, log())
{
}

CPHDXMLControl::CPHDXMLControl(logging::Logger* log, bool ownLog) :
    six::XMLParser(CPHD_URI, false, log, ownLog),
    mCommon(CPHD_URI, false, CPHD_URI, log)
{
}
...
XMLElem CPHDXMLControl::toXML(const Global& global, XMLElem parent)
{
    ...
    // The "size" attribute isn't used in cphd
