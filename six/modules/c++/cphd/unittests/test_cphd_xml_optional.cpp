...
TEST_CASE(testOptional)
{
    ...
    TEST_ASSERT_EQ(errorParams.monostatic->posVelErr.PositionDecorr().value().corrCoefZero, 0.5);
    TEST_ASSERT_EQ(errorParams.monostatic->posVelErr.PositionDecorr().value().decorrRate, 1.0);
    ...
    TEST_ASSERT_EQ(errorParams.monostatic->tropoError->TropoRangeVertical().value(), 5.0);
    TEST_ASSERT_EQ(errorParams.monostatic->tropoError->TropoRangeSlant().value(), 5.0);
    const auto& tropoRangeDecorr = errorParams.monostatic->tropoError->TropoRangeDecorr().value();
    TEST_ASSERT_EQ(tropoRangeDecorr.corrCoefZero, 0.5);
    TEST_ASSERT_EQ(tropoRangeDecorr.decorrRate, 1.0);
    TEST_ASSERT_EQ(errorParams.monostatic->ionoError->IonoRangeVertical().value(), 5.0);
    TEST_ASSERT_EQ(errorParams.monostatic->ionoError->IonoRangeRateVertical().value(), 5.0);
    TEST_ASSERT_EQ(errorParams.monostatic->ionoError->IonoRgRgRateCC().value(), 0.5);
    const auto& ionoRangeVertDecorr = errorParams.monostatic->ionoError->IonoRangeVertDecorr().value();
