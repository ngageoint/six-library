...
std::vector<types::RowCol<double> > Utilities::computeCornersPix(
        const six::sicd::ComplexData& data)
{
    std::vector<types::RowCol<double> > cornersPix;
    ...
    else
    {
        ...
        cornersPix.push_back(types::RowCol<double>(0.0, 0.0));
        cornersPix.push_back(types::RowCol<double>(
...
types::RowCol<scene::Vector3> Utilities::deriveUnitVectors(
        const six::sicd::ComplexData& data)
{
    std::auto_ptr<scene::SceneGeometry> geometry(
            six::sicd::Utilities::getSceneGeometry(&data));

    types::RowCol<scene::Vector3> unitVectors;
    const scene::Vector3 slantPlaneX = geometry->getSlantPlaneX();
    const scene::Vector3 normal = geometry->getGroundPlaneNormal();
...
types::RowCol<double> Utilities::deriveSpacing(
        const six::sicd::ComplexData& data,
        ...
        const types::RowCol<scene::Vector3>& unitVectors)
{
    ...
    std::auto_ptr<scene::ProjectionModel> projection(
            six::sicd::Utilities::getProjectionModel(&data, geometry.get()));

    std::vector<types::RowCol<double> > cornersPix = computeCornersPix(data);
    std::vector<scene::Vector3> corners(cornersPix.size());
...
types::RgAz<std::vector<double> > Utilities::computeMetersFromCenter(
        const six::sicd::ComplexData& data,
        const types::RowCol<scene::Vector3>& unitVectors)
{
    std::vector<scene::Vector3> inPlaneCornersECEF =
...
types::RowCol<size_t> Utilities::derivePlaneDimensions(
...
types::RowCol<double> Utilities::deriveReferencePoint(
        const types::RgAz<std::vector<double> >& sortedMetersFromCenter,
        const types::RowCol<double>& spacing,
        const six::sicd::AreaPlane& areaPlane)
{
    types::RowCol<double> referencePoint;
...
void Utilities::setAreaPlane(six::sicd::ComplexData& data,
        bool includeSegmentList)
{
    if (!data.radarCollection.get())
    {
        data.radarCollection.reset(new six::sicd::RadarCollection());
    }
    if (!data.radarCollection->area.get())
    {
    ...
    if (!data.radarCollection->area->plane.get())
    {
        ...
        deriveAreaPlane(data, includeSegmentList);
    }
}

void Utilities::deriveAreaPlane(const six::sicd::ComplexData& data,
        six::sicd::AreaPlane& areaPlane)
{
    areaPlane.xDirection.reset(new six::sicd::AreaDirectionParameters());
    areaPlane.yDirection.reset(new six::sicd::AreaDirectionParameters());

    const types::RowCol<scene::Vector3> unitVectors = deriveUnitVectors(data);
    areaPlane.xDirection->unitVector = unitVectors.row;
    areaPlane.yDirection->unitVector = unitVectors.col;

    const types::RowCol<double> spacing = deriveSpacing(data, unitVectors);
...
void  Utilities::deriveAreaPlane(six::sicd::ComplexData& data,
        bool includeSegmentList)
{
    ...
    if (includeSegmentList)
    {
        ...
        areaPlane.segmentList[0].reset(new six::sicd::Segment());
        areaPlane.segmentList[0]->startLine = 0;
        areaPlane.segmentList[0]->startSample = 0;
        areaPlane.segmentList[0]->endLine = areaPlane.xDirection->elements - 1;
