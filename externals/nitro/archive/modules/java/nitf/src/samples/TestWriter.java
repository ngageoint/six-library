...
public class TestWriter
{
    ...
    private static int _width = WIDTH;

    private static int _height = HEIGHT;

    private static int _bands = BANDS;
    ...
    public static void main(String[] args) throws nitf.NITFException
    {
        ...
        for (int i = 0; i < list.size(); i++)
        {
            ...
            else if (arg.equals("-w") && i < list.size() - 1)
            {
                _width = Integer.parseInt((String) list.get(++i));
                _width = Math.max(_width, 2);
            }
            else if (arg.equals("-h") && i < list.size() - 1)
            {
                _height = Integer.parseInt((String) list.get(++i));
                _height = Math.max(_height, 2);
            }
            else if (arg.equals("-b") && i < list.size() - 1)
            {
                _bands = Integer.parseInt((String) list.get(++i));
                _bands = Math.max(_bands, 1);
            }
            else if (arg.equals("--help") || arg.equals("--?"))
            {
                System.out
                        .println("usage: java "
                                + TestWriter.class.getName()
                                + " [-h height] [-w width] [-b bands] [-o name] [--help | --?]");
                System.out.println("    Where:");
                System.out
                        .println("-------------------------------------------------------");
                System.out
                        .println("width      ............    sets the image width");
                System.out
                        .println("height     ............    sets the image height");
                System.out
                        .println("bands      ............    sets the number of bands");
                System.out
    ...
    private static ImageSource makeImageSource() throws NITFException
    {
        // create an IOHandle to open the file read-only
        IOHandle handle = new IOHandle(FILENAME, IOHandle.NITF_ACCESS_READONLY,
        ...
        {
            FileSource source = new FileSource(handle, i, 1, _bands - 1);
    ...
    private static void writeTestImage() throws nitf.NITFException
    {

        IOHandle handle = new IOHandle(FILENAME,
                IOHandle.NITF_ACCESS_WRITEONLY, IOHandle.NITF_CREATE);
        if (!handle.isValid())
        {
            throw new nitf.NITFException("Invalid IO Handle");
        }

        byte[] buf = new byte[_width * _height * _bands];

        /* fill the buffer in indrements of 256 */
        for (int i = 0; i < _height; ++i)
        {
            for (int j = 0; j < _width; ++j)
            {
                ...
                {
                    buf[_width * _bands * i + _bands * j + k] = (byte) (j % 256);
                }
            }
        }

        handle.write(buf, buf.length);
        handle.close();
    }
