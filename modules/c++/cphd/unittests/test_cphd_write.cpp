...
void buildRandomMetadata(cphd::Metadata& metadata)
{
    ...
    {
        metadata.data.arraySize.push_back(cphd::ArraySize(
    ...
    metadata.global.imageArea.plane.reset(new cphd::AreaPlane());
    ...
    //! Dummy antenna
    metadata.antenna.reset(new cphd::Antenna());
...
void addFXParams(cphd::Metadata& metadata)
{
...
void addTOAParams(cphd::Metadata& metadata)
{
...
{
    metadata.data.numCPHDChannels = NUM_IMAGES;
    cphd::CPHDWriter writer(metadata, NUM_THREADS);

    cphd::VBM vbm(metadata.data, metadata.vectorParameters);
    ...
    cphd::CPHDReader reader(FILE_NAME, NUM_THREADS);
