...
int main(int argc, char** argv)
{
    try
    {
        ...
        six::NITFReadControl* reader = new six::NITFReadControl;
        ...
        {
        	six::sicd::ComplexData* data = reinterpret_cast<six::sicd::ComplexData*>(container->getData(0));
        ...
        {
        	six::sidd::DerivedData* data = reinterpret_cast<six::sidd::DerivedData*>(container->getData(0));
        	projection.reset(six::sidd::Utilities::getProjectionModel(data));
        	geom = (six::sidd::Utilities::getSceneGeometry(data));
        	std::cout << geom->getReferencePosition() << std::endl;
        	imagePt = pixelToImagePoint(*data, pixelLoc);
        } else {
        	std::cerr << sicdPathname << " is not a SICD or SIDD\n";
        	return 1;
        }
        ...
        double timeCOA(0.0);
        std::cout << "Converted imagePt : " <<  imagePt.row << ", " << imagePt.col << std::endl;
        ...
        types::RowCol<double> imageReturnPoint = projection->sceneToImage(groundPt1, &timeCOA );

        std::cout << "Image Point : " << imageReturnPoint.row << ", " << imageReturnPoint.col << std::endl;

        types::RowCol<double> imageSCP = projection->sceneToImage(geom->getReferencePosition(), & timeCOA);

        std::cout << "Image SCP Point : " << imageSCP.row << ", " << imageSCP.col << std::endl;
