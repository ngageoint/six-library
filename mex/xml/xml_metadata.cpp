 ...
 *  back.
 *
 *  Note that this is built with the C++ bindings and xml.lite from CODA,
 *  which means that Matlab can have issues with unresolved symbols or the
 *  name mangling in general, so this should be always built with the
 *  same compiler as matlab was (which in most cases is GCC, except on
 *  Windows, where you want to use Visual Stuio)
 *
 */
